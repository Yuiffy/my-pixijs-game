...
const expectedRows = [
  ["3 本", "3", "5", "75%", "25%", "0%", "0%", "0%"],
  ["4 本", "4", "9", "50%", "38%", "11%", "1%", "0%"],
  ["5 本", "5", "14", "35%", "35%", "24%", "5%", "1%"],
  ["6 本", "6", "20", "25%", "30%", "30%", "13%", "2%"],
  ["7 本", "7", "27", "15%", "25%", "32%", "23%", "5%"],
  ["8 本", "8", "36", "10%", "20%", "30%", "30%", "10%"],
  ["9 本", "9", "46", "7%", "15%", "25%", "35%", "18%"],
  ["10 本", "10", "满级", "5%", "10%", "20%", "40%", "25%"],
];

const { chromium } = loadPlaywright();
const baseUrl = process.env.AUTOCHESS_BASE_URL || "http://127.0.0.1:3100";
const artifactDirectory = ".tmp/autochess/shop-odds";
...
(async () => {
  ...
  await dialog.getByRole("button", { name: "商店概率", exact: true }).click();
  await dialog.getByRole("heading", { name: "各等级商店抽取概率" }).waitFor();

  const rows = await dialog.locator("tbody tr").evaluateAll((elements) =>
    elements.map((row) => Array.from(row.querySelectorAll("td"), (cell) => cell.textContent?.trim() || "")),
  );
  assert.deepEqual(rows, expectedRows);
  assert.match(await dialog.innerText(), /满级仍保留低费棋/);
  ...
  const screenshotPath = `${artifactDirectory}/shop-odds-codex.png`;
  const screenshotMetrics = inspectPng(await page.screenshot({ path: screenshotPath, fullPage: true }));

  await page.setViewportSize({ width: 390, height: 844 });
  await page.waitForTimeout(250);
  const mobileLayout = await dialog.locator("main").evaluate((element) => ({
    clientWidth: element.clientWidth,
    scrollWidth: element.scrollWidth,
    clientHeight: element.clientHeight,
    scrollHeight: element.scrollHeight,
  }));
  assert.ok(mobileLayout.clientWidth > 0 && mobileLayout.clientHeight > 0);
  assert.ok(mobileLayout.scrollWidth >= mobileLayout.clientWidth);
  assert.ok(mobileLayout.scrollHeight >= mobileLayout.clientHeight);
  const mobileScreenshotPath = `${artifactDirectory}/shop-odds-codex-mobile.png`;
  const mobileScreenshotMetrics = inspectPng(await page.screenshot({ path: mobileScreenshotPath, fullPage: true }));
  ...
  console.log(JSON.stringify({
    ...
    rows,
    canvas: canvasInfo,
    screenshot: { path: screenshotPath, ...screenshotMetrics },
    mobile: {
      layout: mobileLayout,
      screenshot: { path: mobileScreenshotPath, ...mobileScreenshotMetrics },
    },
    errors,
    failedResponses,
  }, null, 2));

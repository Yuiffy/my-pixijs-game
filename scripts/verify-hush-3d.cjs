...
async function solve(page, photos = false) {
  ...
  for (let i = 0; i < 40; i++) {
    ...
    const photograph = photos && !capturedActions.has(key) ? `night-${s.level + 1}-${key}` : undefined;
...
async function main() {
  ...
  try {
    ...
    await advance(page, 0); await capture(page, 'ready');
    await page.locator('#hush-start').click(); await capture(page, 'first-person');
    for (let level = 0; level < 6; level++) {
      await solve(page, true); await capture(page, `result-${level + 1}`);
...
module.exports = { state, advance, follow, hold, cover, solve, capture, images, run: main };

...
(async () => {
  ...
  async function pageFor(g, mobile = false) {
    ...
    await ctx.addInitScript(state => { localStorage.setItem('reset-rush-v1', JSON.stringify(state)); if (window.speechSynthesis) window.speechSynthesis.speak = () => {}; }, g);
  ...
  try {
    const p = await pageFor(E.createGame(84, 'lean'));
    await p.getByRole('button', { name: /管理/ }).click(); await p.getByRole('button', { name: '补 $80 → PRO 100', exact: true }).click();
    ...
    await p.getByRole('button', { name: /管理/ }).click(); await p.getByRole('button', { name: /开新号/ }).nth(2).click();
    await p.getByRole('button', { name: /管理/ }).click(); await p.getByRole('button', { name: /开新号/ }).nth(0).click();
    g = await read(p); assert.equal(g.players[0].accounts.length, 3); assert.equal(g.players[0].cash, 180); assert.equal(g.actions, 0);
    await p.locator('#end-day').click(); await p.locator('#next-day').click();
    ...
    await lp.locator('[data-mode="luna"]').focus(); await lp.keyboard.press('Enter');
    await lp.locator('#develop').focus(); await lp.keyboard.press('Enter');
    g = await read(lp); assert.equal(g.players[0].projects[0].work, 2); assert.equal(g.players[0].accounts[0].quota, 0); assert.equal(g.actions, 2);
    ...
    const dual = E.createGame(42, 'dual'); dual.day = 7; dual.event = { ...E.EVENTS[2] };
    ...
    assert.equal((await read(mp)).players[0].projects[0].work, 11);

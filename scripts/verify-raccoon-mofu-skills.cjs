...
(async () => {
  ...
  try {
    ...
    const setup = await page.evaluate(() => {
      ...
      const pkEffect = battle.effects.find((effect) => effect.kind === "pk_overheat");
      pkEffect.life = pkEffect.maxLife * 0.56;
      ...
      return {
        ...
        maxHp: raccoon.maxHp,
        taunted: battle.enemy.filter((fighter) => fighter.tauntedByFid === raccoon.fid).length,
      };
    });
    assert.ok(Math.abs(setup.shield - setup.maxHp * 0.18) < 0.001);
    assert.equal(setup.taunted, 2);
    const pkText = JSON.parse(await page.evaluate(() => window.render_game_to_text()));
    assert.ok(
      pkText.battle.visualEffects.effects.some((effect) => effect.kind === "pk_overheat"),
    );
    await capture("raccoon-auto-pk.png");

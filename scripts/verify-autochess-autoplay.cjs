...
(async () => {
  ...
  assert.equal(await toolbar.getByText(/v0\.2\.2/).count(), 0);
  ...
  await settings.getByRole("button", { name: /版本与更新.*v0\.2\.2/ }).click();
  const releaseNotes = page.getByRole("dialog", { name: /v0\.2\.2/ });
  ...
  assert.ok(await settings.getByRole("button", { name: /版本与更新.*v0\.2\.2/ }).isVisible());

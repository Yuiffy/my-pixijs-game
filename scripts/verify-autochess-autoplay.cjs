...
(async () => {
  ...
  assert.equal(await toolbar.getByText(/v0\.2\.1/).count(), 0);
  ...
  await settings.getByRole("button", { name: /版本与更新.*v0\.2\.1/ }).click();
  const releaseNotes = page.getByRole("dialog", { name: /v0\.2\.1/ });
  ...
  assert.ok(await settings.getByRole("button", { name: /版本与更新.*v0\.2\.1/ }).isVisible());

...
(async () => {
  ...
  const balancedStyle = strategyGroup.getByRole("radio", { name: "均衡" });
  const highrollStyle = strategyGroup.getByRole("radio", { name: "搏上限" });
  const seerStyle = strategyGroup.getByRole("radio", { name: "看穿" });
  const seer2Style = strategyGroup.getByRole("radio", { name: "看穿2" });
  const goStyle = strategyGroup.getByRole("radio", { name: "Go级" });
  assert.equal(await survivalStyle.getAttribute("aria-checked"), "true");
  await balancedStyle.click();
  assert.equal((await state()).interface.autoplayStyle, "balanced");
  await highrollStyle.click();
  assert.equal((await state()).interface.autoplayStyle, "highroll");
  assert.equal((await state()).interface.autoplayInformationMode, "normal");
  await seerStyle.click();
  assert.equal((await state()).interface.autoplayStyle, "seer");
  assert.equal((await state()).interface.autoplayInformationMode, "oracle");
  await seer2Style.click();
  assert.equal((await state()).interface.autoplayStyle, "seer2");
  assert.equal((await state()).interface.autoplayInformationMode, "oracle");
  await goStyle.click();
  assert.equal((await state()).interface.autoplayStyle, "go");
  assert.equal((await state()).interface.autoplayInformationMode, "oracle");
  await survivalStyle.click();
  assert.equal((await state()).interface.autoplayStyle, "survival");
  assert.equal((await state()).interface.autoplayInformationMode, "normal");
  ...
  await settings.getByRole("button", { name: /版本与更新.*v0\.2\.3/ }).click();
  const releaseNotes = page.getByRole("dialog", { name: /v0\.2\.3/ });
  ...
  assert.ok(await settings.getByRole("radio", { name: "看穿" }).isVisible());
  assert.ok(await settings.getByRole("radio", { name: "看穿2" }).isVisible());
  assert.ok(await settings.getByRole("radio", { name: "Go级" }).isVisible());
  assert.equal(await settings.getByRole("switch", { name: "天眼商店" }).count(), 0);
  ...
  assert.ok(await settings.getByRole("button", { name: /版本与更新.*v0\.2\.3/ }).isVisible());

...
(async () => {
  ...
  const oracleSwitch = settings.getByRole("switch", { name: "天眼商店" });
  ...
  assert.equal(await oracleSwitch.getAttribute("aria-checked"), "false");
  await oracleSwitch.click();
  ...
  assert.ok(await settings.getByRole("switch", { name: "天眼商店" }).isVisible());

...
const usage = `Usage: node scripts/autochess-portrait-generate.mjs --unit <unit-id> --prompt-file <path> [--count <1-4>] [--size <1024x1024>] [--model <model-id>]

Requires OPENAI_API_KEY, ANTHROPIC_API_KEY, or ANTHROPIC_AUTH_TOKEN. Set OPENAI_BASE_URL or ANTHROPIC_BASE_URL for an OpenAI-compatible proxy. Generated candidates are written to .tmp/autochess-portraits/<unit-id>/ and are never published automatically.`;

const imageApiKey = process.env.OPENAI_API_KEY || process.env.ANTHROPIC_API_KEY || process.env.ANTHROPIC_AUTH_TOKEN;
const configuredApiBaseUrl = process.env.OPENAI_BASE_URL || process.env.ANTHROPIC_BASE_URL || "https://api.openai.com";
const imageApiBaseUrl = configuredApiBaseUrl.replace(/\/v1\/?$/, "").replace(/\/$/, "");
...
const main = async () => {
  ...
  const model = readOption(args, "--model", "gpt-image-1");
  const background = readOption(args, "--background", "transparent");
  ...
  if (!["transparent", "opaque"].includes(background)) throw new Error("--background must be transparent or opaque.");
  if (!imageApiKey) {
    throw new Error("Set OPENAI_API_KEY, ANTHROPIC_API_KEY, or ANTHROPIC_AUTH_TOKEN in your shell or local environment; do not commit it.");
  }
  ...
  for (let index = 1; index <= count; index += 1) {
    const response = await fetch(`${imageApiBaseUrl}/v1/images/generations`, {
      method: "POST",
      headers: {

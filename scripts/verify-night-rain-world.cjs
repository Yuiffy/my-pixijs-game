...
async function main(){
 ...
 await use('courtyard');assert.equal((await state(page)).checkpoint,'courtyard');assert.equal(await page.evaluate(()=>JSON.parse(localStorage.getItem('night-rain-v1')).checkpoint),'courtyard');await capture(page,'02-courtyard-recorded');
 // Receive one actual hit, then use the usual pilot to disengage/finish the foe.
 ...
 await travel({x:0,y:0,z:8});const before=(await state(page)).player;await page.keyboard.press('r');await advance(page,450);assert.equal((await state(page)).player.action,'heal');await capture(page,'03-drink-medicine');await advance(page,800);assert.ok((await state(page)).player.hp>before.hp);assert.equal((await state(page)).player.flasks,before.flasks-1);checks.push('discoverable medicine HUD, real R healing, lamp writes checkpoint immediately');
 await travel({x:-30.5,y:0,z:-3.8});await capture(page,'04-water-gate-below');assert.equal((await state(page)).templeGate,false);
 ...
 await travel({x:-32,y:3.6,z:-18});await capture(page,'08-temple-descent');await use('temple-lamp');assert.equal((await state(page)).checkpoint,'temple-lamp');assert.equal((await state(page)).player.flasks,4);await capture(page,'09-lotus-lamp');
 await use('temple-gate');assert.equal((await state(page)).templeGate,true);assert.equal(await page.evaluate(()=>JSON.parse(localStorage.getItem('night-rain-v1')).templeGate),true);await page.waitForTimeout(900);await capture(page,'10-water-gate-open');await travel({x:-22,y:0,z:-2});await use('courtyard');checks.push('west loop via high bridge, flask upgrade, temple lamp and genuinely traversable return gate');
 await use('canal-lamp');assert.equal((await state(page)).checkpoint,'canal-lamp');
 ...
 const levelBefore=(await state(page)).level;await page.evaluate(()=>{window.virtualPad.buttons[12]={pressed:true,value:1};});await advance(page,20);assert.equal((await state(page)).level,levelBefore+1);await page.evaluate(()=>{window.virtualPad.buttons[12]={pressed:false,value:0};});await advance(page,20);checks.push('virtual gamepad D-pad upgrade at the new ferry shrine');
 await capture(page,'11-ferry-refuge');await page.keyboard.press('m');await capture(page,'12-expanded-map');await page.getByRole('button',{name:'关闭',exact:true}).click();
 await page.evaluate(()=>window.nightRain.save());const saved=await state(page);await page.reload({waitUntil:'networkidle'});await page.getByRole('button',{name:'继续雨夜旅程 →',exact:true}).click();await advance(page,0);const restored=await state(page);assert.equal(restored.checkpoint,'canal-lamp');assert.equal(restored.templeGate,true);assert.equal(restored.player.flasks,4);assert.deepEqual(restored.collected,saved.collected);
 await page.addScriptTag({content:pilot+';window.nightRainPilot={chooseInput,NIGHT_ROUTE};'});
 await travel({x:4,y:0,z:-37});await advance(page,5000,{});for(let i=0;i<15&&(await state(page)).mode==='playing';i++)await advance(page,3000,{});assert.equal((await state(page)).mode,'dead');await capture(page,'13-death');await page.getByRole('button',{name:'回到雨灯',exact:true}).click();await advance(page,0);assert.equal((await state(page)).player.x,17);assert.equal((await state(page)).player.z,-30);assert.equal((await state(page)).player.flasks,4);await capture(page,'14-respawn-at-ferry');checks.push('all lamps, persistent doors/medicine, reload, genuine death and selected checkpoint respawn');

  ...
  await page.evaluate(() => {
    ...
    while (hook) {
      if (hook.memoizedState?.current?.engine?.state) {
        window.__shioriVerificationEngine = hook.memoizedState.current.engine;
        return;
      }
      hook = hook.next;
    }
...
const artifactDirectory = ".tmp/autochess/shiori-stun";
...
(async () => {
  ...
  try {
    await page.goto(`${baseUrl}/game/autochess?seed=901`, {
    ...
    const setup = await page.evaluate(() => {
      const engine = window.__shioriVerificationEngine;
      engine.state.round = 6;
      engine.state.playerLevel = 4;
      engine.state.board.fill(null);
      engine.state.board[0] = { uid: 1, id: "shiori", star: 3 };
      engine.startBattle();
      ...
      battle.enemy.forEach((fighter, index) => {
        fighter.x = index < 3 ? 455 + index * 42 : 880 + index * 18;
        fighter.y = index < 3 ? 380 : 190 + index * 70;
      ...
      engine.update(0.05);
      const landing = shiori.abilityMotion;
      const cluster = [
        { x: landing.toX, y: landing.toY - 70 },
        { x: landing.toX, y: landing.toY + 70 },
        { x: landing.toX + 70, y: landing.toY },
      ];
      battle.enemy.slice(0, cluster.length).forEach((fighter, index) => {
        fighter.x = cluster[index].x;
        fighter.y = cluster[index].y;
      });
      return {
        star: shiori.star,
    ...
    assert.equal(setup.star, 3);
    assert.equal(setup.attackType, "melee");
    assert.equal(setup.motion?.abilityId, "shiori");
    ...
    const hitEnemies = impact.enemies.filter((fighter) => fighter.damageTaken > 0);
    assert.ok(impact.shield > setup.shield);
    assert.ok(hitEnemies.length >= 3, JSON.stringify(impact));
    assert.ok(
      hitEnemies.every((fighter) => fighter.stun > 0.8 && fighter.stun <= 1),
      JSON.stringify(hitEnemies),
    );

    const textState = JSON.parse(await page.evaluate(() => window.render_game_to_text()));
    const textHitEnemies = textState.battle.enemyUnits.filter((fighter) => fighter.damageTaken > 0);
    assert.equal(textHitEnemies.length, hitEnemies.length);
    assert.ok(
      textHitEnemies.every((fighter) => fighter.stun > 0.8 && fighter.stun <= 1),
      JSON.stringify(textHitEnemies),
    );

    const screenshotPath = `${artifactDirectory}/shiori-three-star-impact.png`;
    const screenshot = await page.screenshot({ path: screenshotPath, fullPage: true });
    const screenshotMetrics = inspectPng(screenshot);
    ...
    console.log(JSON.stringify({
      ...
      hitEnemies,
      textHitEnemies,
      screenshot: { path: screenshotPath, bytes: screenshot.length, metrics: screenshotMetrics },

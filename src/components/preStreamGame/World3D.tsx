...
import { STATIONS, TASK_IDS } from "./gameplay3d";
import type { PrepState, StationId } from "./gameplay3d";
...
function StationMarker({ id, position, label, done, active, near, ready, onClick }: {
  id: StationId; position: Vec3; label: string; done: boolean; active: boolean; near: boolean; ready: boolean; onClick: (id: StationId) => void;
}) {
  const [hovered, setHovered] = useState(false);
  const ring = useRef<THREE.Mesh>(null);
  const color = ready ? "#65d5a5" : done ? "#8dc8a6" : active ? "#e49b86" : near ? "#eed58b" : "#b8acc3";
  useFrame(({ clock }) => {
    if (ring.current) {
      const pulse = ready || active || near || hovered ? 1 + Math.sin(clock.elapsedTime * 4) * 0.08 : 1;
      ring.current.scale.setScalar(pulse);
    }
  });
  const visible = id !== "spill" && id !== "cable" && id !== "catwalk" ? true : active;
  if (!visible) return null;
  return (
    <group position={position}>
      <mesh ref={ring} rotation={[-Math.PI / 2, 0, 0]}>
        <ringGeometry args={[0.34, 0.39, 32]} />
        <meshBasicMaterial color={color} transparent opacity={done && !ready ? 0.38 : 0.82} depthWrite={false} />
      </mesh>
      <mesh position={[0, 0.115, 0]}>
        <sphereGeometry args={[0.065, 12, 9]} />
        <meshStandardMaterial color={color} emissive={color} emissiveIntensity={near || active || ready ? 0.55 : 0.1} />
      </mesh>
      {(hovered || near || ready) && <StationLabel station={id} title={ready ? "开始直播" : label} done={done && !ready} />}
      <mesh position={[0, 0.72, 0]} onClick={(event) => { event.stopPropagation(); onClick(id); }} onPointerOver={(event) => { event.stopPropagation(); setHovered(true); document.body.style.cursor = "pointer"; }} onPointerOut={() => { setHovered(false); document.body.style.cursor = ""; }}>
        <cylinderGeometry args={[0.53, 0.53, 1.42, 12]} />
...
function StationLabel({ station, title, done }: { station: StationId; title: string; done: boolean }) {
  const texture = useMemo(() => {
    ...
    const ctx = canvas.getContext("2d");
    if (ctx) {
      ctx.fillStyle = done ? "#385d52" : "#4c405b";
      ctx.beginPath(); ctx.roundRect(9, 10, 238, 64, 12); ctx.fill();
      ctx.fillStyle = "#fff8ed";
      ctx.font = 'bold 28px "Microsoft YaHei", sans-serif';
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText(done ? `${title} ✓` : title, 128, 43);
    }
  ...
  useEffect(() => () => texture.dispose(), [texture]);
  const thermos = station === "thermos";
  return <sprite position={[thermos ? 0.4 : 0, 1.8, 0]} scale={thermos ? [1, 0.36, 1] : [1.18, 0.42, 1]}><spriteMaterial map={texture} transparent depthTest={false} /></sprite>;
}
...
function Room({ state, liveStateRef, onStationClick }: { state: PrepState; liveStateRef: MutableRefObject<PrepState>; onStationClick: (id: StationId) => void }) {
  ...
  return (
    <>
      ...
      {incidents.includes("spill") && <group position={[0.15, 0.02, 0.25]}>{disk([0, 0, 0], 0.42, "#7bbfce")}<Orb at={[0.23, 0.045, 0.06]} scale={[0.07, 0.025, 0.06]} color="#abedf1" transparent opacity={0.8} /></group>}
      {incidents.includes("cable") && <Tube from={[4.45, 0.07, 0.35]} to={[3.0, 0.06, 0.15]} radius={0.026} color="#d2787e" />}
      {incidents.includes("catwalk") && <Orb at={[3.1, 0.18, -1.2]} scale={[0.18, 0.08, 0.14]} color="#e3bb98" />}
      {STATIONS.map((station) => {
        ...
        return <StationMarker key={station.id} id={station.id} position={[station.x, 0.035, station.z]} label={station.label} done={done} active={incidents.includes(station.id as (typeof incidents)[number])} near={distance < 1.45 && !incidentHasPriority} ready={station.id === "obs" && readyToGoLive} onClick={onStationClick} />;
      })}
...
export default function World3D({ state, liveStateRef, onStationClick, onReady }: World3DProps) {
  return (
    <Canvas
      shadows
      orthographic
    ...
    >
      <Room state={state} liveStateRef={liveStateRef} onStationClick={onStationClick} />
    </Canvas>
  );
}

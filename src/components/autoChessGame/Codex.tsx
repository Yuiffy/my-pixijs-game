      ...
      <main style={{ flex: 1, overflow: "auto", padding: 18 }}>
        ...
        {tab === "rules" && (
          <section style={{ maxWidth: 820, lineHeight: 1.9 }}>
            ...
            <h2>经济</h2>
            <p>没有固定工资：每场战斗按敌方完整阵容结算，1/2/3 星敌人分别提供 1/2/3 金，无论胜负都会全额发放；实际击败数只计入战斗统计。每 5 金提供 1 利息，最多计算 20 金，即最高 4 利息；理财Ⅱ改为每 4 金提供 1 利息，最多 20 利息，需要存到 80 金。连胜和理财还能提供额外收入。升本是“支付当前剩余费用直接升级”；每回合的自然减费最低可将费用降至 0，溢出减费会结转到后续本级。最高 10 本可上阵 10 人。</p>
          </section>
        )}
      </main>
    </div>

        ...
        {tab === "odds" && (
          <section>
            ...
            <ul style={{ color: "#a8bdca", lineHeight: 2 }}>
              ...
              <li>达到 10 本后，升本入口会变成升星工坊；花 40 金解锁后可直升一星或二星棋子。</li>
            </ul>
          </section>
        )}
        {tab === "rules" && (
          <section style={{ maxWidth: 820, lineHeight: 1.9 }}>
            <h2>远征与无限裂隙</h2>
            <p>守住前 16 战即完成远征；第 4、8、12 战是精英关，第 16 战是终局首领，备战阶段会提前显示醒目的预警。普通关会保留储蓄空间，精英与首领才是集中投入战力的检查点。第 17—31 战为普通无限，敌军按 5 利息、连胜和上一战阵容结算金全部复投；第 22 战起敌军会逐步突破 10 人上限。第 32 战起进入地狱无限，改按 20 利息、理财固定收入、连胜与结算金全部复投，敌方人数此后不设硬上限。无限首领会轮换时停、续航和高费压制编队；敌军会组成并激活羁绊，预算优先转化为人口和升星，剩余预算才转化为额外属性。</p>
            <h2>操作</h2>
            ...
            <h2>经济</h2>
            <p>没有固定工资：每场战斗按敌方完整阵容结算，1/2/3 星敌人分别提供 1/2/3 金，无论胜负都会全额发放；实际击败数只计入战斗统计。每 5 金提供 1 利息，最多计算 20 金，即最高 4 利息；理财Ⅱ改为每 4 金提供 1 利息，最多 20 利息，需要存到 80 金。连胜和理财还能提供额外收入。升本是“支付当前剩余费用直接升级”；每回合的自然减费最低可将费用降至 0，溢出减费会结转到后续本级。最高 10 本可上阵 10 人。满本后可花 40 金解锁升星工坊；一星直升费用为 4 ×（棋子费用 + 1），二星直升费用为 12 ×（棋子费用 + 1），三星不可继续直升。</p>
          </section>
        )}
      </main>
    </div>

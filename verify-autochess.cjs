...
(async () => {
  ...
  await page.getByRole('button', { name: /火热整活/ }).click();
  ...
  await page.getByRole('button', { name: /锁定商店|已锁定/ }).click();
  ...
  await page.locator('.rift-dom-shop-card:not(.disabled)').nth(0).click();
  await page.locator('.rift-dom-shop-card:not(.disabled)').nth(0).click();
  prep = await state();
